...
def main():
    ...
    opx_boot = OpxBootstrap(server, cli_args)
    if not os.path.isdir(cli_args.topdir):
        opx_boot.exit_results(dict(
            ok=False,
            error_type='args',
            message='{} is not a directory'.format(cli_args.topdir)))

    opx_boot.log.info("bootstrap init-delay: {} seconds".format(cli_args.init_delay))

    opx_boot.post_device_status(message='bootstrap started, waiting for device access', state='START')

    time.sleep(cli_args.init_delay)
    opx_boot.wait_for_device(countdown=cli_args.reload_delay, poll_delay=10)

    opx_boot.log.info("proceeding with bootstrap")

    opx_boot.check_os_install_and_finally()
    opx_boot.log.info("bootstrap process finished")
    opx_boot.exit_results(dict(ok=True))
